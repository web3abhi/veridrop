...
contract ClaimFactory is AccessControl, Initializable {
    ...
    function initialize(address claimImplementation, uint256 _claimFee, uint256 _claimPrice, bytes calldata networkId)
        external
        initializer
    {
    ...
    function deployClaimContract(
        ...
        address newClaimContract = address(new ProxyContract(_claimImplementation, _owner, _initializer));
        ...
        if (!_claimSettings.hasAllowanceMechanism) {
            IERC20(_claimSettings.airdropToken).safeTransferFrom(
                msg.sender, newClaimContract, _claimSettings.claimAmountDetails.totalClaimAmount
            );
        }

        emit NewClaimContract(newClaimContract);
        ClaimEmitter(_emitterContract).claimContractDeployed(
            _claimSettings, totalWallets, blockNumber, whitelistNetwork, _networkId, address(newClaimContract)
        );
    }
    ...
    function setEmitter(address _emitter) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function changeClaimPrice(uint256 _newClaimPrice) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function changeClaimFee(uint256 _newClaimFee) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function changeClaimImplementation(address _newClaimImplementation) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function withdrawFunds(address _receiver, uint256 _amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
